...
> Current known build state: the notification entry point is currently shown in the **bottom navigation bar**, not the top navigation bar assumed in the original draft of this plan. Backend notification dispatch is confirmed working and email delivery is occurring, but notifications are still not being delivered/rendered in the mobile app, so data-driven notification-list checks remain pending.
...
| 2 | **[CHECK]** if there is an unread count badge on the notification entry point | Badge shows count if unread notifications exist; hidden if 0 | Pending | Backend dispatch/email delivery is working, but no notifications are reaching the mobile app, so unread-count behavior cannot yet be verified in-app |
| 3 | **[ACTION]** Tap the notification entry point | Notification List screen opens successfully | Pass | Screen opens successfully |
| 4 | **[CHECK]** screen layout: search bar, filter control, notification cards or empty state | Available UI elements are present and properly laid out for the current data state | Pass | Search bar and filter are present; despite confirmed backend/email dispatch, the mobile app still shows the empty state instead of delivered notification cards |
...
| 6 | **[ACTION]** Tap different filter chips / filter options | List filters accordingly; "All" shows everything | Pending | Filtering behavior remains pending because backend dispatch/email works but no notification items are delivered to the mobile app |
| 7 | **[ACTION]** Tap a notification card (if any exist) | Card visually marks as read (blue dot disappears, background changes); navigates to linked content if applicable | Pending | No delivered notification cards appear in the mobile app even though notification dispatch is working on the backend/email path |
| 8 | **[CHECK]** each notification card shows: category icon, title, message preview, timestamp, read/unread indicator | All fields render without clipping or overflow | Pending | No notification cards are available to inspect in the mobile app because dispatched notifications are not being delivered/rendered there |
| 9 | **[ACTION]** Pull down to refresh | Loading indicator appears; list refreshes |  | Pull-down to refresh is currently missing on the notification list |
| 10 | **[ACTION]** Scroll to bottom of list | Pagination loads more items (or empty state if no more) | Pending | Pagination remains pending because the notification list stays empty in the mobile app despite confirmed backend/email dispatch |
| 11 | **[ACTION]** If "Clear Read" / "Mark All as Read" is visible, tap it | Bulk actions are visible when expected and, once data exists, should update read state accordingly | Pending | `Clear Read` and `Mark All as Read` are visible in the UI, but their behavior cannot be validated until dispatched notifications are actually delivered to the mobile app |
...
| 5 | **[ACTION]** Toggle Email Notifications OFF then ON | Toggle animates; brief "Saved"/"Saving..." feedback appears; no crash | Pass | Toggle updates successfully and is confirmed working, including backend-side state update |
| 6 | **[ACTION]** Toggle Push Notifications OFF then ON | Same as above | Pass | Toggle updates successfully and is confirmed working, including backend-side state update |
...
| **Help & Support link** | | | | |
| 16 | **[ACTION]** Tap "Help & Support" from Settings | Navigates to Help & Support Hub (tested in TC-M-006) | Pass | Settings > Help & Support routes correctly to the Help & Support hub |
...
| 2 | **[CHECK]** if reviews exist: sort options visible (Most Recent / Rating High→Low / Rating Low→High) | Sort control is present | Pass | Sorting control is visible with 4 modes: highest rating, lowest rating, newest, and oldest |
...
| 5 | **[ACTION]** Tap a sort option | List reorders accordingly | Pass | Sort options work and the list can be reordered using the available highest rating, lowest rating, newest, and oldest modes |
...
| 4 | **[CHECK]** form fields: card number, cardholder name, expiry date, CVV, optional billing address, nickname, "Set as default" toggle | All fields present with proper input types |  | Observed gateway form shows card number, expiry/CVC, billing region/ZIP, Link option, setup action, and `Set as default` toggle; cardholder name and nickname were not confirmed from the captured add-card form in this round |
...
| 12 | **[ACTION]** Tap "Edit" on a card | Edit form opens with pre-filled editable fields (nickname, billing address); card number shown as masked read-only |  | Payment Method Details opens and shows masked card data plus editable metadata fields; the `Set as default payment method` switch in Edit also works, but Method Nickname save still does not persist even though success toast appears; current evidence indicates the nickname is not saved to the database and the payment list label never changes from the card brand |
...
| 13 | **[ACTION]** Tap "Cancel" | Modal dismisses; returns to flow without submitting |  | No separate final-confirmation modal / cancel action is present in the current accepted flow |
...
| 9 | **[DATA]** If Articles load: check subtype filter (Tutorial Guides / Troubleshooting Tips) and article cards | List renders; tapping an article opens detail with title, body, "Was this helpful?" |  | Articles list renders with `All`, `Tutorial Guides`, and `Troubleshooting` filters; article detail opens with title, body, table of contents, related articles, and `Helpful / Not Helpful`; article feedback now works, but the subtype tabs are still currently UI-only and do not functionally filter content |
| 10 | **[ACTION]** If search bar is functional: type a query | Results appear or "No results" message with Contact Support CTA | Pass | Help Center search now accepts input and works correctly |
...
| **Contact Support** | | | | |
| 12 | **[ACTION]** Tap "Contact Support" | Contact Support Form opens | Pass | Tapping `Contact Support` now routes correctly into the support-ticket flow via the My Support Tickets list, where a `Create New Ticket` action is available |
| 13 | **[CHECK]** form fields: Title, Category picker, Description, Priority picker, Attachments | All fields present with correct input types | Pass | `Create New Ticket` form is present with Case Title, Category, Description, Priority selection, Submit, and Cancel controls; this is accepted as compliant for the current documented flow |
| 14 | **[CHECK]** category options include: Technical Issue, Account Access, Payment Question, Booking Issue, General Inquiry, Feature Request, Bug Report, Feedback | Options match spec (no admin-only categories like "Provider Support") | Pass | Category picker opens and shows the current supported options, including Account Access, Billing & Payments, Appointments & Scheduling, Technical Issue, and Aftercare Support; this is accepted as compliant for the current documented flow |
| 15 | **[CHECK]** priority options: Low, Medium, High, Urgent with inline descriptions | Options and helper text present | Pass | Priority options Low, Medium, High, and Urgent are visible on the Create New Ticket form |
| 16 | **[ACTION]** Try to submit with empty fields | Submit button disabled or shows validation errors | Pass | Validation is present: submitting with empty required fields shows inline/toast validation such as `Please enter a case title` and `Please select a category` |
| 17 | **[ACTION]** Fill all required fields and submit | Confirmation screen with Case ID (CASE-YYYY-#####) and "We will respond within 24 hours" message; then navigates to My Support Tickets |  | Creating a new support ticket works via `My Support Tickets > Create New Ticket`, and the newly created ticket appears in the ticket list in-session, but it does not persist after exiting and reopening the app |
| **My Support Tickets** | | | | |
...
| 23 | **[CHECK]** reply input field visible for Open/In Progress/Resolved tickets; hidden for Closed | Input field state matches ticket status |  | Reply capability is available in ticket detail and attachments can be added in replies, but the closed-ticket hidden-state path was not separately exercised in this round |
...
| 11 | **[CHECK]** read-only view: all fields display as non-editable text; passport number is masked; "Contact Support to Request a Correction" button present | Record is locked; fields match what was entered |  | Submitted passport read-only state was not evidenced in the current captures |
...
| 19 | Navigate to the **"In Progress" view** | View loads; Travel Itinerary screen is accessible | Pass | In Progress-view travel itinerary content is evidenced in the provided screenshot set |
| 20 | **[ACTION]** Open Travel Itinerary screen | Itinerary View loads with title "Travel Itinerary" | Pass | The itinerary screen loads and shows booking context plus travel sections; a dedicated `Travel Itinerary` title is not visible in the provided captures, but the itinerary content itself is clearly evidenced |
| 21 | **[CHECK]** itinerary layout: booking context header, flight sections (outbound/return), hotel section | Sections present; submitted records show summary data; pending records show "not yet submitted" prompts | Pass | Booking context header is visible together with submitted `Outbound Flight` and `Hotel Information` sections, while `Return Flight` is shown as not yet submitted with a `Submit Return Flight` CTA |
| 22 | **[ACTION]** Tap a submitted record section | Navigates to read-only detail view |  | This exact itinerary-to-detail transition was not separately captured in the current round |
| 23 | **[ACTION]** Tap back | Returns to itinerary |  | Back-to-itinerary behavior was not separately evidenced in the current round |
...
| 2 | RC-01 | **Delete Account rate-limit:** Enter wrong password/OTP multiple times during identity verification | Rate-limit / lockout message appears after configured threshold; retry blocked |  | Wrong password / wrong OTP validation is confirmed, and OTP flow enforces a resend limit of 3 requests within 30 minutes; full lockout-threshold behavior was not exhaustively reproduced in this round |
...
| 5 | RC-03 | **Change Password:** Navigate to Settings > Privacy & Security > Change Password | Change password form loads; enter current + new password; submit succeeds |  | Change password works and the new password can be used to log in, but validation incorrectly allows the new password to be the same as the current password |
...
| 7 | RC-06 | **Payment Methods nickname:** Add a card with a custom nickname (e.g., "Personal Visa") | On the Payment Methods list, the card displays the custom nickname instead of just the brand name |  | Editing Method Nickname shows a success toast, but the list still displays the card brand, reopening the detail/edit screen shows the nickname field blank again, and current verification indicates the nickname is not saved to the database |
...
| 9 | RC-11 | **My Reviews regression:** Open My Reviews list → tap a review → edit → save; also test Request Takedown | Edit saves and publishes immediately; Takedown shows confirmation prompt |  | Request Takedown can be submitted, and individual aspect ratings can now be changed independently, but the edit/save path still has the long-running `Saving` bug |
...
| 11 | RC-12 | **Notification swipe actions:** On the Notification List, swipe left on an unread notification | Swipe reveals "Mark as Read" and "Archive" action buttons | Pending | Backend dispatch/email delivery is confirmed working, but notifications are not being delivered to the mobile app, so swipe actions remain pending |
| 12 | RC-12 | **Notification archive:** Tap "Archive" on the swipe action | Notification hidden from default list; retrievable via "All" filter | Pending | Archive behavior remains pending because dispatched notifications never appear in the mobile app list |
...
|--------|-----------------|---------|---------|---------|-------|
| Notifications | Bell icon + badge | | Yes | | Notification entry point is present in bottom nav, but unread badge behavior remains pending because dispatched notifications are not being delivered to the mobile app |
| Notifications | Notification list + filters | | Yes | | Screen opens successfully and filter UI is visible, but the app still shows the empty state despite confirmed backend/email dispatch; pull-down to refresh is currently missing |
| Notifications | Swipe actions (mark read, archive) | | Yes | | Still pending because dispatched notifications never appear in the mobile app list to swipe on |
| Notifications | Deep link navigation | | Yes | | Pending because no delivered notifications are available in the mobile app to open and deep-link from |
| Notifications | Search | Yes | | | Search bar is visible on the notification screen |
| Settings | Main screen (4 nav rows) | | | | |
| Settings | Notification preferences (toggles) | Yes |  | | Both Email Notifications and Push Notifications toggles now update successfully and are confirmed working, including backend-side state update |
...
| My Reviews | Reviews list + sort | Yes |  | | Review list is present and sorting works with 4 modes: highest rating, lowest rating, newest, and oldest |
| My Reviews | Review detail view | Yes | | | Detail view loads and shows rating breakdown plus detailed review content |
| My Reviews | Edit review | | Yes | | Edit flow exists; individual aspect ratings can now be changed independently, but save can still get stuck in `Saving` state |
| My Reviews | Submit review photo upload | | |  | Completed-case submit-review screen has no visible image-upload control, while edit-review screen exposes a Photos section |
| My Reviews | Submit/edit review UI consistency | |  | | Submit-review screen and edit-review screen are not structurally aligned |
...
| Payment Methods | Edit card metadata | |  | | Detail/edit screen is reachable, but Method Nickname does not persist after save even though a success toast is shown; current verification indicates the nickname is not saved to the database and the list label remains the card brand |
...
| Help & Support | Help Center browser (Articles) | | Yes | | Articles list/detail views load, Help Center search now works, and `Helpful / Not Helpful` feedback now works; however, `Tutorial Guides` / `Troubleshooting` tabs are still UI-only and do not functionally filter content |
| Help & Support | Help Center browser (Resources) | |  | | Resources list/detail screens are present, but currently appear to be UI-level only; functional actions are not confirmed as working |
| Help & Support | Help Center browser (Videos) | |  | | Video Tutorials list/detail screens are present, but currently appear to be UI-level only; functional actions are not confirmed as working |
| Help & Support | Contact Support form | Yes |  | | `Contact Support` now routes correctly into the support-ticket flow via My Support Tickets, where `Create New Ticket` provides the submission entry point; the form shows Case Title, Category, Description, and Priority options and is accepted as compliant for the current documented flow |
| Help & Support | My Support Tickets list | | Yes | | Ticket list loads and status tabs filter correctly, and a newly created support ticket appears in the list in-session, but it disappears after exiting and reopening the app |
...
| Travel | Passport read-only view | |  | | Submitted passport read-only state was not checked in this UI-only passport-form round, so record-lock / masked-passport rendering remains unverified |
...
| Travel | Flight submission (return) | | Yes | | `Return Flight` is now evidenced in the In Progress itinerary view with a `Return flight not yet submitted` prompt and `Submit Return Flight` CTA, but the separate return-flight submission form itself was not opened in this round |
...
| Travel | Travel Itinerary view | Yes |  | | In Progress itinerary view is now evidenced with booking context, submitted outbound-flight details, pending return-flight prompt, and submitted hotel-information details |
...
| Delete Account | Rate-limit on identity verification (RC-01) | |  | | Wrong password / wrong OTP validation is confirmed, and OTP resend is limited to 3 requests within 30 minutes; broader lockout-threshold coverage still incomplete |
...
| Payment Methods | Nickname display on list (RC-06) | |  | | Bug persists: success toast appears, but the list still shows the card brand, the nickname field is blank again on reopen, and current verification indicates the nickname is not saved to the database |
...
| Notifications | Swipe actions (RC-12) | | Yes | | Still pending because backend/email dispatch works, but notifications are not being delivered to the mobile app for swipe-action retest |
...
| Date | Tester | TC Completed | Pass |  |  | Defects Filed | Notes |
|------|--------|-------------|------|------|---------|---------------|-------|
| | | | | | | | |
